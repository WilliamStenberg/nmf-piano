...
double calculateNorm(double *a, double *w, double *h, double * d, int m, int n, int k)
{
  ...
  dlacpy('A', m, n, a, m, d, m);

  //d = - w*h
  dgemm('N', 'N', m, n, k, - 1.0, w, m, h, k, 1., d, m);


  //dnorm = frobnorm(d) / sqrt(m*n)
  dnorm = dlange('F', m, n, d, m, NULL);
  dnorm = dnorm / sqrt(m*n);

  double hnorm = 0;
  hnorm = dlange('F', k, n, h, k, NULL);

  dnorm = dnorm + hnorm * LASSO_LAMBDA;
